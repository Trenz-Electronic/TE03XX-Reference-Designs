...
//====================================================
	...
	while(1)
	{
		...
		if (character > 1) 
		{
			switch (character) {
				case 'a':
					RAM_test(XPAR_DDR_SDRAM_MPMC_HIGHADDR+MEMORY_OFFSET, 60*1024*256,1); //60MB
					break;
				case 'f':
					RAM_test(XPAR_DDR_SDRAM_MPMC_HIGHADDR+MEMORY_OFFSET, 60*1024*256,0); //60MB
					break;
				case 'c':
					cache=~cache;
					caching(cache);
				...
				case 't':
					...
					DBG(xil_printf("\r\nRunning TX test\r\n");)
					tx_run = 1;
					tx_addr=XPAR_DDR_SDRAM_MPMC_HIGHADDR+MEMORY_OFFSET;
				...
				case 's':
					if (rx_run==1) {
						...
						DBG(xil_printf("\r\nrx_addr 0x%08X\r\n", rx_addr);)
						DBG(xil_printf("\r\nReceived %d bytes\r\n", rx_addr-XPAR_DDR_SDRAM_MPMC_HIGHADDR-MEMORY_OFFSET);)
						if (XST_SUCCESS == RAM_test(DMA.WrStartAddr,((rx_addr-XPAR_DDR_SDRAM_MPMC_HIGHADDR-MEMORY_OFFSET)>>2), 2)) //verify only
