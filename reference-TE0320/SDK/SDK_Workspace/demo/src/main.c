		...
		if (character > 1) 
		{
			switch (character) {
				...
				case 'c':
					...
					break;
				case 'g':	
					xil_printf("\r\nBoard revision value: 0x%03X", XIo_In32(XPAR_SWITCHES_BASEADDR)>>10);
					xil_printf("\r\nPush button 2 value: %d", 1&(XIo_In32(XPAR_SWITCHES_BASEADDR)>>8));
					xil_printf("\r\nPower fail (inv) value: %d", 1&(XIo_In32(XPAR_SWITCHES_BASEADDR)>>9));
					xil_printf("\r\nDIP Switches value: 0x%02X\r\n", 0xFF&XIo_In32(XPAR_SWITCHES_BASEADDR));
					break;
				case 't':
