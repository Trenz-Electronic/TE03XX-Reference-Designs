...
void menu(void) {
	...
	xil_printf("    'g' prints switches state and board revision\r\n");
...
{	
	...
	XGpio_mSetDataReg(XPAR_LED_BASEADDR, 1, 0xF);
...
//MAIN LOOP
//====================================================
//	RAM_test(XPAR_DDR_SDRAM_MPMC_BASEADDR+MEMORY_OFFSET, 120*1024*256,3); //just fill memory 120MB
//	xil_printf("\r\nMemory filled\r\n");
	...
	while(1)
	{
		...
		if (1 != XUartLite_mIsReceiveEmpty(STDIN_BASEADDRESS)) 
		{ 	
		...
		if (character > 1) 
		{
			switch (character) {
				case 'a':
					RAM_test(XPAR_DDR_SDRAM_MPMC_BASEADDR+MEMORY_OFFSET, 120*1024*256,1); //120MB
					break;
				case 'f':
					RAM_test(XPAR_DDR_SDRAM_MPMC_BASEADDR+MEMORY_OFFSET, 120*1024*256,0); //120MB
					break;
				case 'c':
					cache=~cache;
					caching(cache);
					break;
				case 'g':	
					xil_printf("\r\nBoard revision value: 0x%03X", XGpio_mGetDataReg(XPAR_SWITCHES_BASEADDR, 1)>>10);
					xil_printf("\r\nPush button 2 value: %d", 1&(XGpio_mGetDataReg(XPAR_SWITCHES_BASEADDR, 1)>>8));
					xil_printf("\r\nPower fail (inv) value: %d", 1&(XGpio_mGetDataReg(XPAR_SWITCHES_BASEADDR, 1)>>9));					
					xil_printf("\r\nDIP Switches value: 0x%02X\r\n", 0xFF&XGpio_mGetDataReg(XPAR_SWITCHES_BASEADDR, 1));
					break;
				case 't':
